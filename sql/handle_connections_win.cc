...
struct Socket_Listener: public Listener
{
  ...
  */
  void begin_accept()
  {
    ...
    sql_print_error("my_AcceptEx failed", GetLastError());
    abort();
  }
...
{
  Pipe_Listener():
    Listener(INVALID_HANDLE_VALUE, CreateEvent(0, TRUE, FALSE, 0))
  {
  }
  /*
...
/* Wait  for max 5 handles 2 sockets (base_ip,extra_ip), 2 pipes, and shutdown event*/
#define MAX_WAIT_HANDLES 5
...
void handle_connections_win()
{
  ...
  {
    /*
      Use minimum 2 listeners for pipe, to prevent ERROR_PIPE_BUSY on client side.
     (ensure that there is always at least 1 ConnectNamedPipe() call inflight)
    */
    for (int i= 0; i < 2; i++)
